...
def gvf_control_boat_2(p,dot_p,e,n,H,ke,kd,s,m,mua,mux,muy,F,R):
    """
    Parameters
    ----------
    p : 2D vector
    ...
    dot_pdhat=dot_pd/npl.norm(dot_pd)
    dot_Xd = ddot_pdhat.T.dot(E).dot(dot_pdhat)
    
    mu_inv=np.array([[1/mux,0],[0,1/muy]])
    ux=([1],[0])
    dot_phat=dot_p/npl.norm(dot_p)
    

    ghi = dot_pdhat.T.dot(R).dot(mu_inv).dot(ux)
    phi_dot = dot_Xd + kd*(dot_pdhat.T.dot(E).dot(dot_phat))
    #Esta es la parte que no funciona y por eso la tengo comentada
